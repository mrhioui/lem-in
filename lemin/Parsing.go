...
func CompletTunnul(line string, Rooms []Room, Start string, End string) {
	parts := strings.Split(line, "-")
	if len(parts) != 2 {
	...
	for _, RoomA := range Rooms {
		for _, RoomB := range Rooms {
			if parts[0] == RoomA.Name && parts[1] == RoomB.Name {
